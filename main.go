...
func main() {
    db, err := gorm.Open(postgres.Open("host=db user=postgres password=postgres dbname=todos sslmode=disable"), &gorm.Config{})
    if err != nil {
        log.Fatal("データベース接続に失敗しました:", err)
    }

    db.AutoMigrate(&models.Todo{}, &models.Tag{}, &models.User{}, &models.TodoTag{})
